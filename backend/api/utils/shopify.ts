// ============================================================================
//  PATCHED shopify.ts  —  duplicate-order dedup + null-guard fixes
// ----------------------------------------------------------------------------
//  WHAT CHANGED vs. GitHub main (as of Aug 12 2026):
...
//    5. NEW (Aug 12 2026) — calculateFinalAmount + calculateDraftOrder
//       now null-guard the calculatedDraftOrder before returning it.
//       When Shopify returns null (unshippable address / missing variant
//       / transient API hiccup) we now return { error, code, details }
//       instead of a raw object with null in it. The controller uses this
//       to send a HELPFUL 400 to the customer instead of crashing with
//       "Cannot read properties of null (reading 'totalPriceSet')".
//       Aug 12 log had 13 such crashes = ~2 stranded customers.
//
...
export const calculateDraftOrder = async (payload: Payload) => {
    ...
    try {
        ...
        // ── NULL GUARD (Aug 12 2026) ──
        // Shopify sometimes returns calculatedDraftOrder=null for unshippable
        // addresses, missing variants, or transient API issues. Previously we
        // returned { data } with the null inside, and the caller crashed on
        // reads like data.draftOrderCalculate.calculatedDraftOrder. Now we
        // treat null as a soft error so the controller can send 400 instead
        // of the frontend crashing.
        if (!data?.draftOrderCalculate?.calculatedDraftOrder) {
            console.error(
                "[calculateDraftOrder] Shopify returned no calculatedDraftOrder — null response",
                { address: shippingAddress, lineItemCount: cartItems.length },
            );
            return {
                error:
                    "Shopify could not calculate this order (null response). Address may be unshippable, or a line item is invalid.",
...
export const calculateFinalAmount = async (payload: Payload) => {
    ...
    try {
        ...
        // ── NULL GUARD (Aug 12 2026) — the fix for the 500 crash ──
        // Was: return { data: data.draftOrderCalculate.calculatedDraftOrder }
        // When Shopify returned calculatedDraftOrder=null we handed the null
        // back to the controller, which then crashed reading .totalPriceSet.
        // Now: return a soft error so the controller can send 400 with a
        // human-readable message instead of a raw 500.
        const calc = data?.draftOrderCalculate?.calculatedDraftOrder;
        if (!calc) {
            console.error(
                "[calculateFinalAmount] Shopify returned no calculatedDraftOrder — null response",
                { address: shippingAddress, lineItemCount: cartItems.length },
            );
            return {
                error:
                    "Shopify could not calculate the final amount (null response). Address may be unshippable, or a line item is invalid.",
                code: "shopify_calc_returned_null",
            };
        }
